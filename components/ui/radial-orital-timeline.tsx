      ...
      <div className="relative w-full max-w-4xl h-full flex items-center justify-center">
        ...
        >
          <div className="absolute w-16 h-16 rounded-full bg-gradient-to-br from-primary via-primary/80 to-primary/60 animate-pulse flex items-center justify-center z-10">
            ...
            ></div>
            <div className="w-8 h-8 rounded-full bg-background border-2 border-primary"></div>
          </div>
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{
                    background: `radial-gradient(circle, hsl(var(--primary) / 0.2) 0%)`,
                ...
                <div
                  ...
                  ${isExpanded ? "dark:text-white text-foreground scale-125" : "dark:text-white/70 text-muted-foreground"}
                `}
                >
                  {item.title}
                </div>
                  ...
                  <Card className="absolute top-20 left-1/2 -translate-x-1/2 w-64 bg-card/95 backdrop-blur-lg border-border shadow-xl overflow-visible">
                    ...
                    <CardHeader className="pb-2">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-xs font-mono dark:text-white/70 text-muted-foreground">
                          {item.date}
                        </span>
                      </div>
                      <CardTitle className="text-sm mt-2 dark:text-white text-foreground">
                        {item.title}
                      </CardTitle>
                    </CardHeader>
                    <CardContent className="text-xs dark:text-white/80 text-muted-foreground">
                      <p>{item.content}</p>

                      <div className="mt-4 pt-3 border-t border-border">
                        <div className="flex justify-between items-center text-xs mb-1">
                          <span className="flex items-center dark:text-white text-foreground">
                            <Zap size={10} className="mr-1 text-primary" />
                            Energy Level
                          </span>
                          <span className="font-mono dark:text-white text-foreground">{item.energy}%</span>
                        </div>
                      ...
                      {item.relatedIds.length > 0 && (
                        <div className="mt-4 pt-3 border-t border-border">
                          <div className="flex items-center mb-2">
                            <Link size={10} className="dark:text-white/70 text-muted-foreground mr-1" />
                            <h4 className="text-xs uppercase tracking-wider font-medium dark:text-white text-foreground">
                              Connected Nodes
                            </h4>
                          </div>
                            ...
                            {item.relatedIds.map((relatedId) => {
                              ...
                              return (
                                <Button
                                  ...
                                  variant="outline"
                                  size="sm"
                                  className="flex items-center h-6 px-2 py-0 text-xs rounded-sm"
                                  onClick={(e) => {
                                    e.stopPropagation();
