...
const NameAnimation = () => {
    ...
    const lastName = 'MADHURA';
    const finalText = `${firstName} ${lastName}`;
    const [displayText, setDisplayText] = useState('A' + finalText.slice(1).replace(/[A-Z]/g, ' ')); // Start with N visible
    ...
    return (
        <span className="inline-flex select-none"> {/* Added select-none to prevent text selection during animation */}
            ...
            <span className="mx-2"></span> {/* Consistent spacing between names */}
